...
public class FTensorDense extends AbstractTensor<Float, FStorageArray, FTensor> implements FTensor {
    ...
    public FTensorDense(TensorManager manager, Shape shape, FStorageArray storage, Order order) {
        super(manager, shape);
    ...
    @Override
    public FTensorDense reshape(Shape shape, Order askOrder) {
        if (shape.size() != this.shape.size()) {
            throw new IllegalArgumentException("Incompatible shape size.");
        }
        return new FTensorDense(manager, shape, storage, askOrder);
    }

    @Override
    public FTensor t() {
        int[] reversedShape = IntArrays.reverse(Arrays.copyOf(shape.dims(), shape.rank()));
        return new FTensorDense(manager, Shape.of(reversedShape), storage,
    ...
    public FTensor copy(Order askOrder) {
        if (askOrder == order) {
            return new FTensorDense(manager, shape, storage.copy(), order);
        }
        return FTensor.super.copy(askOrder);
    }
