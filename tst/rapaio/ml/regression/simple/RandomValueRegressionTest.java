...
public class RandomValueRegressionTest {
    ...
    public void testRandomValueRegression() {
        RegResult fit1 = RandomValueRegression.newRVR().fit(df, father).predict(df);
        RegResult fit2 = RandomValueRegression.from(Normal.of(10, 0.1)).fit(df, father).predict(df);
    ...
    public void testNaming() {
        ...
        assertEquals("Normal(mu=10, sd=20)", RandomValueRegression.from(Normal.of(10, 20)).newInstance().distribution().name());
