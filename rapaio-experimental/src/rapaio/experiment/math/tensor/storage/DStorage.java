...
public class DStorage implements Storage<Double> {
    ...
    public void swap(int a, int b, final int n) {
        ...
        for (; i < loopBound; i += SPECIES_LEN, a += SPECIES_LEN, b += SPECIES_LEN) {
            DoubleVector va = DoubleVector.fromArray(SPECIES, array, a);
            DoubleVector vb = DoubleVector.fromArray(SPECIES, array, b);
    ...
    public void reverse(int start, int len) {
        ...
        VectorShuffle<Double> shuffle = SPECIES.shuffleFromValues(indexes);

        while (tail - head >= step) {
            DoubleVector hv = DoubleVector.fromArray(SPECIES, array, head);
            DoubleVector tv = DoubleVector.fromArray(SPECIES, array, tail - SPECIES_LEN);
    ...
    private int cmp(double a, double b, boolean asc) {
        int sign = asc ? 1 : -1;
