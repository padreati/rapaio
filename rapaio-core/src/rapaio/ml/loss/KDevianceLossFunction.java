 ...
 */
public class KDevianceLossFunction implements LossFunction {

    @Serial
    private static final long serialVersionUID = 3608607822562742621L;
    private final int k;

    public KDevianceLossFunction(int k) {
        this.k = k;
    }
    ...
    public boolean equalOnParams(LossFunction object) {
        if (!(object instanceof KDevianceLossFunction)) {
            return false;
        }
        return Integer.valueOf(k).equals(((KDevianceLossFunction) object).k);
    }
}
