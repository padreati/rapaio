...
public final class VectorizedITensorStride extends rapaio.math.tensor.mill.barray.BaseITensorStride implements ITensor {
    ...
    public VectorizedITensorStride(VectorizedArrayTensorMill mill, Shape shape, int offset, int[] strides, int[] array) {
        this(mill, StrideLayout.of(shape, offset, strides), array);
    }

    public VectorizedITensorStride(VectorizedArrayTensorMill mill, Shape shape, int offset, Order order, int[] array) {
        this(mill, StrideLayout.ofDense(shape, offset, order), array);
    }

    public VectorizedITensorStride(VectorizedArrayTensorMill mill, StrideLayout layout, int[] array) {
    ...
    @Override
    public VectorizedITensorStride apply(Order askOrder, IntIntBiFunction<Integer> apply) {
    ...
    public Integer vdot(ITensor tensor, int start, int end) {
        ...
        VectorizedITensorStride dts = (VectorizedITensorStride) tensor;
                ...
                futures.add(service.submit(() -> {
                    for (int c = 0; c < p; c += innerChunk) {
                        ...
                        for (int k = 0; k < n; k += vectorChunk) {
                            ...
                            for (int i = rs; i < re; i++) {
                                var krow = (VectorizedITensorStride) rows.get(i);
                                for (int j = c; j < ce; j++) {
    ...
    public ITensor copy(Order askOrder) {
        ...
        VectorizedITensorStride dst = (VectorizedITensorStride)
                mill.ofInt().stride(StrideLayout.ofDense(shape(), 0, askOrder), copy);
    ...
    public ITensor copyTo(ITensor to, Order askOrder) {
        ...
        if (to instanceof VectorizedITensorStride dst) {
                        ...
                        if (loop) {
                            if (level == slices.length) {
                                ...
                                futures.add(executor.submit(() -> {
                                    VectorizedITensorStride s = (VectorizedITensorStride) this.narrowAll(false, ss, es);
                                    VectorizedITensorStride d = (VectorizedITensorStride) dst.narrowAll(false, ss, es);
    ...
    private void directCopyTo(VectorizedITensorStride src, VectorizedITensorStride dst, Order askOrder) {
