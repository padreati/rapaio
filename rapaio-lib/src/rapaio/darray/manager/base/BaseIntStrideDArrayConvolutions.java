...
public final class BaseIntStrideDArrayConvolutions {
    ...
    public static DArray<Integer> unfold1d(DArray<Integer> in, int kLen, int stride, int padding, int dilation) {
        ...
        DArray<Integer> result = input.dm().zeros(DType.INTEGER, Shape.of(n, inCh, kLen, outLen));
    ...
    public static DArray<Integer> conv2d(DArray<Integer> in, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        ...
        DArray<Integer> output = in.dm().zeros(DType.INTEGER, Shape.of(n, outChannels, outH, outW));
    ...
    public static DArray<Integer> unfold2d(DArray<Integer> in, int kH, int kW, int stride, int padding, int dilation) {
        ...
        DArray<Integer> result = input.dm().zeros(DType.INTEGER, Shape.of(n, inC, kH, kW, outLenH, outLenW));
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < inC; c++) {
    ...
    public static DArray<Integer> convTranspose2d(DArray<Integer> input, DArray<?> w, DArray<?> bias, int stride, int padding, int dilation, int groups,
        ...
        DArray<Integer> output = input.dm().zeros(DType.INTEGER, Shape.of(n, outChannels, outH, outW));
    ...
    public static DArray<Integer> conv3d(DArray<Integer> input, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        ...
        DArray<Integer> output = input.dm().zeros(DType.INTEGER, Shape.of(n, outChannels, outD, outH, outW));
        for (int batch = 0; batch < n; batch++) {
            DArray<?> inBatch = input.selsq(0, batch);
            DArray<?> outBatch = output.selsq(0, batch);
            for (int group = 0; group < groups; group++) {
    ...
    public static DArray<Integer> unfold3d(DArray<Integer> input, int kD, int kH, int kW, int stride, int padding, int dilation) {
        ...
        DArray<Integer> result = input.dm().zeros(DType.INTEGER, Shape.of(n, inCh, kD, kH, kW, outD, outH, outW));
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < inCh; c++) {
    ...
    public static Pair<DArray<Integer>, DArray<Integer>> maxPool1d(DArray<Integer> input, int kSize, int stride, int padding, int dilation, boolean ceilMode) {
        ...
        DArray<Integer> output = input.dm().zeros(DType.INTEGER, Shape.of(n, channels, outLen));
                ...
                for (int o = 0; o < outLen; o++) {
                    ...
                    for (int k = 0; k < kSize; k++) {
                        ...
                        if (inPos >= 0 && inPos < inLen) {
                            int val = input.getInt(b, c, inPos);
                            if (val > maxVal) {
                                maxVal = val;
                                maxIdx = inPos;
                            }
    ...
    public static DArray<Integer> maxUnpool1d(DArray<Integer> input, DArray<Integer> indices, int kSize, int stride, int padding, int outputSize) {
        boolean batched = input.rank() == 3;
        if (input.rank() != 2 && input.rank() != 3) {
            throw new IllegalArgumentException("Input must be a 2D or 3D array.");
        }
        ...
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < channels; c++) {
                for (int i = 0; i < inLen; i++) {
                    ...
                    if (idx >= 0 && idx < outLen) {
                        output.setInt(input.getInt(b, c, i), b, c, idx);
                    }
                }
            }
        }
    ...
    public static Pair<DArray<Integer>, DArray<Integer>> maxPool2d(DArray<Integer> input, int kH, int kW, int stride, int padding, int dilation, boolean ceilMode) {
        ...
        DArray<Integer> output = input.dm().zeros(DType.INTEGER, Shape.of(n, channels, outH, outW));
            ...
            for (int c = 0; c < channels; c++) {
                for (int oh = 0; oh < outH; oh++) {
                    for (int ow = 0; ow < outW; ow++) {
                        ...
                        for (int kh = 0; kh < kH; kh++) {
                            ...
                            if (ih >= 0 && ih < inH) {
                                for (int kw = 0; kw < kW; kw++) {
                                    int iw = ow * stride + kw * dilation - padding;
                                    if (iw >= 0 && iw < inW) {
                                        int val = input.getInt(b, c, ih, iw);
                                        if (val > maxVal) {
                                            maxVal = val;
                                            maxIdx = ih * inW + iw;
                                        }
                        ...
                        output.setInt(maxVal, b, c, oh, ow);
    ...
    public static DArray<Integer> maxUnpool2d(DArray<Integer> input, DArray<Integer> indices, int kH, int kW, int stride, int padding, int outH, int outW) {
        boolean batched = input.rank() == 4;
        if (input.rank() != 3 && input.rank() != 4) {
            throw new IllegalArgumentException("Input must be a 3D or 4D array.");
        }
        ...
        DArray<Integer> output = input.dm().zeros(DType.INTEGER, Shape.of(n, channels, outputH, outputW));
                    ...
                    for (int iw = 0; iw < inW; iw++) {
                        ...
                        if (idx >= 0) {
                            ...
                            if (oh < outputH && ow < outputW) {
                                output.setInt(input.getInt(b, c, ih, iw), b, c, oh, ow);
                            }
                        }
                    }
                }
    ...
    public static Pair<DArray<Integer>, DArray<Integer>> maxPool3d(DArray<Integer> input, int kD, int kH, int kW, int stride, int padding, int dilation, boolean ceilMode) {
        boolean batched = input.rank() == 5;
        ...
        DArray<Integer> output = input.dm().zeros(DType.INTEGER, Shape.of(n, channels, outD, outH, outW));
                ...
                for (int od = 0; od < outD; od++) {
                    for (int oh = 0; oh < outH; oh++) {
                        for (int ow = 0; ow < outW; ow++) {
                                    ...
                                    for (int kh = 0; kh < kH; kh++) {
                                        ...
                                        if (ih >= 0 && ih < inH) {
                                            for (int kw = 0; kw < kW; kw++) {
                                                int iw = ow * stride + kw * dilation - padding;
                                                if (iw >= 0 && iw < inW) {
                                                    int val = input.getInt(b, c, id, ih, iw);
                                                    if (val > maxVal) {
                                                        maxVal = val;
                            ...
                            output.setInt(maxVal, b, c, od, oh, ow);
    ...
    public static DArray<Integer> maxUnpool3d(DArray<Integer> input, DArray<Integer> indices, int kD, int kH, int kW, int stride, int padding, int outD, int outH, int outW) {
        boolean batched = input.rank() == 5;
        if (input.rank() != 4 && input.rank() != 5) {
            throw new IllegalArgumentException("Input must be a 4D or 5D array.");
        }
        ...
        DArray<Integer> output = input.dm().zeros(DType.INTEGER, Shape.of(n, channels, outputD, outputH, outputW));
                        ...
                        for (int iw = 0; iw < inW; iw++) {
                            ...
                            if (idx >= 0) {
                                ...
                                if (od < outputD && oh < outputH && ow < outputW) {
                                    output.setInt(input.getInt(b, c, id, ih, iw), b, c, od, oh, ow);
                                }
                            }
                        }
                    }
