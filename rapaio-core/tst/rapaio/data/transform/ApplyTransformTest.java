...
public class ApplyTransformTest {
    ...
    void spotTest() {
        ...
        df.fapply(ApplyTransform.onSpot(s -> {
            s.setMissing(0);
            s.setMissing(1);
        }, VarRange.all()));
    ...
    void testDouble() {
        ...
        Frame sign2 = df.copy().fapply(ApplyTransform.onDouble(Math::signum, VarRange.all()).newInstance());
    ...
    void testInt() {
        ...
        Frame copy = df.copy().fapply(ApplyTransform.onInt(i -> i + 1, VarRange.all()));
        for (int j = 0; j < df.rowCount(); j++) {
    ...
    void testString() {
        ...
        Frame copy = df.copy().fapply(ApplyTransform.onLabel(l -> "a".equals(l) ? "b" : "a", VarRange.all()));
        for (int j = 0; j < df.rowCount(); j++) {
