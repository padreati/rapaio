...
public final class BaseByteStrideDArrayConvolutions {

    public static DArray<Byte> conv1d(DArray<Byte> input, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation,
            int groups) {
        ...
        for (int batch = 0; batch < n; batch++) {
            ...
            for (int group = 0; group < groups; group++) {
                ...
                DArray<Byte> unfold1d = inSlice.unfold1d(k, stride, padding, dilation);
    ...
    public static DArray<Byte> convTranspose1d(DArray<Byte> input, DArray<?> weights, DArray<?> bias, int stride, int padding, int dilation,
                    ...
                    for (int k = 0; k < kLen; k++) {
                        for (int i = 0; i < inLen; i++) {
                            ...
                            if (outPos >= 0 && outPos < outLen) {
                                outSlice.incByte(col.getByte(oc * kLen + k, i), oc, outPos);
                            }
                        }
                    }
                }
    ...
    public static DArray<Byte> unfold1d(DArray<Byte> in, int kLen, int stride, int padding, int dilation) {
        ...
        DArray<Byte> input = batched ? in : in.stretch(0);
        ...
        DArray<Byte> result = input.dm().zeros(DType.BYTE, Shape.of(n, inCh, kLen, outLen));
    ...
    public static DArray<Byte> conv2d(DArray<Byte> in, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        if (in.rank() > 4) {
    ...
    public static DArray<Byte> unfold2d(DArray<Byte> in, int kH, int kW, int stride, int padding, int dilation) {
        ...
        DArray<Byte> result = input.dm().zeros(DType.BYTE, Shape.of(n, inC, kH, kW, outLenH, outLenW));
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < inC; c++) {
    ...
    public static DArray<Byte> convTranspose2d(DArray<Byte> input, DArray<?> w, DArray<?> bias, int stride, int padding, int dilation, int groups,
            int outputPadding) {
        while (input.rank() < 4) {
            input = input.stretch(0);
        }
    ...
    public static DArray<Byte> conv3d(DArray<Byte> input, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        while (input.rank() < 5) {
            input = input.stretch(0);
        }
        while (kernel.rank() < 5) {
        ...
        DArray<Byte> output = input.dm().zeros(DType.BYTE, Shape.of(n, outChannels, outD, outH, outW));
        for (int batch = 0; batch < n; batch++) {
            DArray<?> inBatch = input.selsq(0, batch);
            DArray<?> outBatch = output.selsq(0, batch);
            for (int group = 0; group < groups; group++) {
    ...
    public static DArray<Byte> unfold3d(DArray<Byte> input, int kD, int kH, int kW, int stride, int padding, int dilation) {
        ...
        DArray<Byte> result = input.dm().zeros(DType.BYTE, Shape.of(n, inCh, kD, kH, kW, outD, outH, outW));
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < inCh; c++) {
    ...
    public static DArray<Byte> convTranspose3d(DArray<Byte> input, DArray<?> weights, DArray<?> bias, int stride, int padding, int dilation,
            int groups,
            int outputPadding) {
        while (input.rank() < 5) {
            input = input.stretch(0);
                                        ...
                                        for (int ih = 0; ih < inH; ih++) {
                                            ...
                                            if (oh >= 0 && oh < outH) {
                                                for (int iw = 0; iw < inW; iw++) {
                                                    ...
                                                    if (ow >= 0 && ow < outW) {
                                                        outSlice.incByte(col.getByte(colRow, id * inH * inW + ih * inW + iw), oc, od,
                                                                oh, ow);
                                                    }
                                                }
                                            }
    ...
    public static Pair<DArray<Byte>, DArray<Integer>> maxPool1d(DArray<Byte> input, int kSize, int stride, int padding, int dilation, boolean ceilMode) {
    ...
    public static DArray<Byte> maxUnpool1d(DArray<Byte> input, DArray<Integer> indices, int kSize, int stride, int padding, int outputSize) {
        boolean batched = input.rank() == 3;
        if (input.rank() != 2 && input.rank() != 3) {
            throw new IllegalArgumentException("Input must be a 2D or 3D array.");
        }
        ...
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < channels; c++) {
                for (int i = 0; i < inLen; i++) {
                    ...
                    if (idx >= 0 && idx < outLen) {
                        output.setByte(input.getByte(b, c, i), b, c, idx);
                    }
                }
            }
        }
    ...
    public static Pair<DArray<Byte>, DArray<Integer>> maxPool2d(DArray<Byte> input, int kH, int kW, int stride, int padding, int dilation, boolean ceilMode) {
        ...
        DArray<Byte> output = input.dm().zeros(DType.BYTE, Shape.of(n, channels, outH, outW));
            ...
            for (int c = 0; c < channels; c++) {
                for (int oh = 0; oh < outH; oh++) {
                    for (int ow = 0; ow < outW; ow++) {
                        ...
                        for (int kh = 0; kh < kH; kh++) {
                            ...
                            if (ih >= 0 && ih < inH) {
                                for (int kw = 0; kw < kW; kw++) {
                                    int iw = ow * stride + kw * dilation - padding;
                                    if (iw >= 0 && iw < inW) {
                                        byte val = input.getByte(b, c, ih, iw);
                                        if (val > maxVal) {
                                            maxVal = val;
                                            maxIdx = ih * inW + iw;
                                        }
                        ...
                        output.setByte(maxVal, b, c, oh, ow);
    ...
    public static DArray<Byte> maxUnpool2d(DArray<Byte> input, DArray<Integer> indices, int kH, int kW, int stride, int padding, int outH, int outW) {
        boolean batched = input.rank() == 4;
        if (input.rank() != 3 && input.rank() != 4) {
            throw new IllegalArgumentException("Input must be a 3D or 4D array.");
        }
        ...
        DArray<Byte> output = input.dm().zeros(DType.BYTE, Shape.of(n, channels, outputH, outputW));
                    ...
                    for (int iw = 0; iw < inW; iw++) {
                        ...
                        if (idx >= 0) {
                            ...
                            if (oh < outputH && ow < outputW) {
                                output.setByte(input.getByte(b, c, ih, iw), b, c, oh, ow);
                            }
                        }
                    }
                }
    ...
    public static Pair<DArray<Byte>, DArray<Integer>> maxPool3d(DArray<Byte> input, int kD, int kH, int kW, int stride, int padding, int dilation, boolean ceilMode) {
        boolean batched = input.rank() == 5;
        ...
        DArray<Byte> output = input.dm().zeros(DType.BYTE, Shape.of(n, channels, outD, outH, outW));
                ...
                for (int od = 0; od < outD; od++) {
                    for (int oh = 0; oh < outH; oh++) {
                        for (int ow = 0; ow < outW; ow++) {
                                    ...
                                    for (int kh = 0; kh < kH; kh++) {
                                        ...
                                        if (ih >= 0 && ih < inH) {
                                            for (int kw = 0; kw < kW; kw++) {
                                                int iw = ow * stride + kw * dilation - padding;
                                                if (iw >= 0 && iw < inW) {
                                                    byte val = input.getByte(b, c, id, ih, iw);
                                                    if (val > maxVal) {
                                                        maxVal = val;
                            ...
                            output.setByte(maxVal, b, c, od, oh, ow);
    ...
    public static DArray<Byte> maxUnpool3d(DArray<Byte> input, DArray<Integer> indices, int kD, int kH, int kW, int stride, int padding, int outD, int outH, int outW) {
        boolean batched = input.rank() == 5;
        if (input.rank() != 4 && input.rank() != 5) {
            throw new IllegalArgumentException("Input must be a 4D or 5D array.");
        }
        ...
        DArray<Byte> output = input.dm().zeros(DType.BYTE, Shape.of(n, channels, outputD, outputH, outputW));
                        ...
                        for (int iw = 0; iw < inW; iw++) {
                            ...
                            if (idx >= 0) {
                                ...
                                if (od < outputD && oh < outputH && ow < outputW) {
                                    output.setByte(input.getByte(b, c, id, ih, iw), b, c, od, oh, ow);
                                }
                            }
                        }
                    }
