...
public class SortVectorTest {
	...
	public void testGetterSetter() throws IOException, URISyntaxException {
		...
		for (int i = 0; i < sort.rowCount(); i++) {
			sort.setValue(i, sort.value(i) + MathBase.E);
		}
		assertEquals(MathBase.E + 1., numeric.value(0), 1e-10);
		assertEquals(MathBase.E + 2.5, numeric.value(2), 1e-10);
		assertEquals(MathBase.E + 4, numeric.value(1), 1e-10);
		assertEquals(MathBase.E + 4., numeric.value(3), 1e-10);
