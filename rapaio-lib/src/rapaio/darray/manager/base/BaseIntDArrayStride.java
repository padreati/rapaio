...
//    }
    ...
    private DArray<Integer> im2col1d(DArray<?> inSlice, int kLen, int padding, int stride, int dilation, int outLen) {
        int inDepth = inSlice.dim(0);
        DArray<Integer> res = inSlice.dm().zeros(DType.INTEGER, Shape.of(inDepth, kLen, outLen));
            ...
            for (int k = 0; k < kLen; k++) {
                for (int o = 0; o < outLen; o++) {
                    ...
                    if (delta >= 0 && delta < inSlice.dim(1)) {
                        res.setInt(inSlice.getInt(i, delta), i, k, o);
                    }
                }
            }
        }
    ...
    @Override
    public DArray<Integer> conv3d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        DArray<Integer> input = this;
        ...
        if (bias != null) {
            for (int oc = 0; oc < outChannels; oc++) output.narrow(1, oc, oc + 1).add_(bias.getInt(oc));
        }
        return output;
    }

    private DArray<Integer> im2col3d(DArray<?> inSlice, int kD, int kH, int kW, int padding, int stride, int dilation, int outD, int outH, int outW) {
        int inDepth = inSlice.dim(0);
        DArray<Integer> res = dm.zeros(DType.INTEGER, Shape.of(inDepth, kD, kH, kW, outD, outH, outW));
        for (int c = 0; c < inDepth; c++) {
                            ...
                            for (int oh = 0; oh < outH; oh++) {
                                for (int ow = 0; ow < outW; ow++) {
                                    ...
                                    if (id >= 0 && id < inSlice.dim(1) && ih >= 0 && ih < inSlice.dim(2) && iw >= 0 && iw < inSlice.dim(3)) {
                                        res.setInt(inSlice.getInt(c, id, ih, iw), c, kd, kh, kw, od, oh, ow);
                                    }
                                }
                            }
                        }
    ...
    @Override
    public DArray<Integer> convTranspose3d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups, int outputPadding) {
        DArray<Integer> input = this;
        while (input.rank() < 5) input = input.stretch(0);
                                    ...
                                    for (int kh = 0; kh < kH; kh++) {
                                        for (int kw = 0; kw < kW; kw++) {
                                            ...
                                            if (od >= 0 && od < outD && oh >= 0 && oh < outH && ow >= 0 && ow < outW) {
                                                for (int oc = 0; oc < outDepth; oc++) {
                                                    outSlice.incInt((int)(val * kernelSlice.getInt(c, oc, kd, kh, kw)), oc, od, oh, ow);
                                                }
                                            }
                                        }
                                    }
