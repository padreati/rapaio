...
//    }
    ...
    private DArray<Double> im2col1d(DArray<?> inSlice, int kLen, int padding, int stride, int dilation, int outLen) {
        int inDepth = inSlice.dim(0);
        DArray<Double> res = inSlice.dm().zeros(DType.DOUBLE, Shape.of(inDepth, kLen, outLen));
            ...
            for (int k = 0; k < kLen; k++) {
                for (int o = 0; o < outLen; o++) {
                    ...
                    if (delta >= 0 && delta < inSlice.dim(1)) {
                        res.setDouble(inSlice.getDouble(i, delta), i, k, o);
                    }
                }
            }
        }
    ...
    @Override
    public DArray<Double> convTranspose2d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups, int outputPadding) {
        DArray<Double> input = this;
        while (input.rank() < 4) {
            input = input.stretch(0);
        }
    ...
    private DArray<Double> im2col2d(DArray<?> inSlice, int kH, int kW, int padding, int stride, int dilation, int outH, int outW) {
        int inDepth = inSlice.dim(0);
        DArray<Double> res = dm.zeros(DType.DOUBLE, Shape.of(inDepth, kH, kW, outH, outW));
        for (int c = 0; c < inDepth; c++) {
            for (int kh = 0; kh < kH; kh++) {
                for (int kw = 0; kw < kW; kw++) {
                    for (int oh = 0; oh < outH; oh++) {
                        for (int ow = 0; ow < outW; ow++) {
                            ...
                            if (ih >= 0 && ih < inSlice.dim(1) && iw >= 0 && iw < inSlice.dim(2)) {
                                res.setDouble(inSlice.getDouble(c, ih, iw), c, kh, kw, oh, ow);
                            }
                        }
                    }
                }
    ...
    public DArray<Double> conv3d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        ...
        if (bias != null) {
            for (int oc = 0; oc < outChannels; oc++) output.narrow(1, oc, oc + 1).add_(bias.getDouble(oc));
        }
        return output;
    }

    private DArray<Double> im2col3d(DArray<?> inSlice, int kD, int kH, int kW, int padding, int stride, int dilation, int outD, int outH, int outW) {
        int inDepth = inSlice.dim(0);
        DArray<Double> res = dm.zeros(DType.DOUBLE, Shape.of(inDepth, kD, kH, kW, outD, outH, outW));
        for (int c = 0; c < inDepth; c++) {
                            ...
                            for (int oh = 0; oh < outH; oh++) {
                                for (int ow = 0; ow < outW; ow++) {
                                    ...
                                    if (id >= 0 && id < inSlice.dim(1) && ih >= 0 && ih < inSlice.dim(2) && iw >= 0 && iw < inSlice.dim(3)) {
                                        res.setDouble(inSlice.getDouble(c, id, ih, iw), c, kd, kh, kw, od, oh, ow);
                                    }
                                }
                            }
                        }
    ...
    @Override
    public DArray<Double> convTranspose3d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups, int outputPadding) {
        DArray<Double> input = this;
        while (input.rank() < 5) input = input.stretch(0);
                                    ...
                                    for (int kh = 0; kh < kH; kh++) {
                                        for (int kw = 0; kw < kW; kw++) {
                                            ...
                                            if (od >= 0 && od < outD && oh >= 0 && oh < outH && ow >= 0 && ow < outW) {
                                                for (int oc = 0; oc < outDepth; oc++) {
                                                    outSlice.incDouble((double)(val * kernelSlice.getDouble(c, oc, kd, kh, kw)), oc, od, oh, ow);
                                                }
                                            }
                                        }
                                    }
