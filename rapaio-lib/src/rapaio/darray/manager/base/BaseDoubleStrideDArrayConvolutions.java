...
public final class BaseDoubleStrideDArrayConvolutions {

    public static DArray<Double> conv1d(DArray<Double> input, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation,
            int groups) {
        ...
        for (int batch = 0; batch < n; batch++) {
            ...
            DArray<Double> inBatch = input.selsq(0, batch); // shape: (inChannels, inLen)
            DArray<Double> outBatch = output.selsq(0, batch); // shape: (outChannels, outLen)

            for (int group = 0; group < groups; group++) {

                DArray<Double> inSlice = inBatch.narrow(0, group * inDepth, (group + 1) * inDepth); // shape: (inDepth, inLen)
                DArray<Double> outSlice = outBatch.narrow(0, group * outDepth, (group + 1) * outDepth); // shape: (outDepth, outLen)
        ...
        if (bias != null) {
            for (int oc = 0; oc < outChannels; oc++) {
                output.narrow(1, oc, oc + 1).add_(bias.getDouble(oc));
            }
        }
        return output;
    }

    public static DArray<Double> convTranspose1d(DArray<Double> input, DArray<?> weights, DArray<?> bias, int stride, int padding, int dilation,
                    ...
                    for (int k = 0; k < kLen; k++) {
                        for (int i = 0; i < inLen; i++) {
                            ...
                            if (outPos >= 0 && outPos < outLen) {
                                outSlice.incDouble(col.getDouble(oc * kLen + k, i), oc, outPos);
                            }
                        }
                    }
                }
    ...
    public static DArray<Double> unfold1d(DArray<Double> in, int kLen, int stride, int padding, int dilation) {
        ...
        DArray<Double> input = batched ? in : in.stretch(0);
                ...
                for (int k = 0; k < kLen; k++) {
                    for (int o = 0; o < outLen; o++) {
                        ...
                        if (delta >= 0 && delta < input.dim(2)) {
                            result.setDouble(input.getDouble(b, c, delta), b, c, k, o);
                        }
                    }
                }
            }
    ...
    public static DArray<Double> conv2d(DArray<Double> in, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        if (in.rank() > 4) {
        ...
        DArray<Double> output = in.dm().zeros(DType.DOUBLE, Shape.of(n, outChannels, outH, outW));
        ...
        try (ExecutorService executor = Executors.newFixedThreadPool(input.dm().cpuThreads())) {
            ...
            for (int batch = 0; batch < n; batch++) {
                int b = batch;
                executor.submit(() -> {
                    DArray<Double> inBatch = input.selsq(0, b);   // (inChannels, inH, inW)
                    DArray<Double> outBatch = output.selsq(0, b); // (outChannels, outH, outW)
                    ...
                    for (int group = 0; group < groups; group++) {
                        ...
                        DArray<Double> col = inSlices.get(group).unfold2d(kH, kW, stride, padding, dilation);
    ...
    public static DArray<Double> unfold2d(DArray<Double> in, int kH, int kW, int stride, int padding, int dilation) {
        ...
        DArray<Double> result = input.dm().zeros(DType.DOUBLE, Shape.of(n, inC, kH, kW, outLenH, outLenW));
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < inC; c++) {
                        ...
                        for (int oh = 0; oh < outLenH; oh++) {
                            for (int ow = 0; ow < outLenW; ow++) {
                                ...
                                if (ih >= 0 && ih < inH && iw >= 0 && iw < inW) {
                                    result.setDouble(input.getDouble(b, c, ih, iw), b, c, kh, kw, oh, ow);
                                }
                            }
                        }
                    }
    ...
    public static DArray<Double> convTranspose2d(DArray<Double> input, DArray<?> w, DArray<?> bias, int stride, int padding, int dilation, int groups,
            int outputPadding) {
        while (input.rank() < 4) {
            input = input.stretch(0);
        }
        ...
        DArray<Double> output = input.dm().zeros(DType.DOUBLE, Shape.of(n, outChannels, outH, outW));
                                    ...
                                    for (int kh = 0; kh < kH; kh++) {
                                        ...
                                        if (oh >= 0 && oh < outH) {
                                            ...
                                            for (int kw = 0; kw < kW; kw++) {
                                                if (ow >= 0 && ow < outW) {
                                                    for (int oc = 0; oc < outDepth; oc++) {
                                                        outSlice.incDouble((double) (val * kernelSlice.getDouble(c, oc, kh, kw)), oc, oh,
                                                                ow);
                                                    }
                                                }
                                                ow += dilation;
    ...
    public static DArray<Double> conv3d(DArray<Double> input, DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        while (input.rank() < 5) {
            input = input.stretch(0);
        }
        while (kernel.rank() < 5) {
        ...
        DArray<Double> output = input.dm().zeros(DType.DOUBLE, Shape.of(n, outChannels, outD, outH, outW));
        for (int batch = 0; batch < n; batch++) {
            DArray<?> inBatch = input.selsq(0, batch);
            DArray<?> outBatch = output.selsq(0, batch);
            for (int group = 0; group < groups; group++) {
    ...
    public static DArray<Double> unfold3d(DArray<Double> input, int kD, int kH, int kW, int stride, int padding, int dilation) {
        ...
        DArray<Double> result = input.dm().zeros(DType.DOUBLE, Shape.of(n, inCh, kD, kH, kW, outD, outH, outW));
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < inCh; c++) {
                                ...
                                for (int oh = 0; oh < outH; oh++) {
                                    for (int ow = 0; ow < outW; ow++) {
                                        ...
                                        if (id >= 0 && id < inD && ih >= 0 && ih < inH && iw >= 0 && iw < inW) {
                                            result.setDouble(input.getDouble(b, c, id, ih, iw), b, c, kd, kh, kw, od, oh, ow);
                                        }
                                    }
                                }
                            }
    ...
    public static DArray<Double> convTranspose3d(DArray<Double> input, DArray<?> weights, DArray<?> bias, int stride, int padding, int dilation,
            int groups,
            int outputPadding) {
        while (input.rank() < 5) {
            input = input.stretch(0);
                                        ...
                                        for (int ih = 0; ih < inH; ih++) {
                                            ...
                                            if (oh >= 0 && oh < outH) {
                                                for (int iw = 0; iw < inW; iw++) {
                                                    ...
                                                    if (ow >= 0 && ow < outW) {
                                                        outSlice.incDouble(col.getDouble(colRow, id * inH * inW + ih * inW + iw), oc, od,
                                                                oh, ow);
                                                    }
                                                }
                                            }
    ...
    public static Pair<DArray<Double>, DArray<Integer>> maxPool1d(DArray<Double> input, int kSize, int stride, int padding, int dilation, boolean ceilMode) {
        ...
        DArray<Double> output = input.dm().zeros(DType.DOUBLE, Shape.of(n, channels, outLen));
                ...
                for (int o = 0; o < outLen; o++) {
                    ...
                    for (int k = 0; k < kSize; k++) {
                        ...
                        if (inPos >= 0 && inPos < inLen) {
                            double val = input.getDouble(b, c, inPos);
                            if (val > maxVal) {
                                maxVal = val;
                                maxIdx = inPos;
                            }
    ...
    public static DArray<Double> maxUnpool1d(DArray<Double> input, DArray<Integer> indices, int kSize, int stride, int padding, int outputSize) {
        boolean batched = input.rank() == 3;
        if (input.rank() != 2 && input.rank() != 3) {
            throw new IllegalArgumentException("Input must be a 2D or 3D array.");
        }
        ...
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < channels; c++) {
                for (int i = 0; i < inLen; i++) {
                    ...
                    if (idx >= 0 && idx < outLen) {
                        output.setDouble(input.getDouble(b, c, i), b, c, idx);
                    }
                }
            }
        }
    ...
    public static Pair<DArray<Double>, DArray<Integer>> maxPool2d(DArray<Double> input, int kH, int kW, int stride, int padding, int dilation, boolean ceilMode) {
        ...
        DArray<Double> output = input.dm().zeros(DType.DOUBLE, Shape.of(n, channels, outH, outW));
            ...
            for (int c = 0; c < channels; c++) {
                for (int oh = 0; oh < outH; oh++) {
                    for (int ow = 0; ow < outW; ow++) {
                        ...
                        for (int kh = 0; kh < kH; kh++) {
                            ...
                            if (ih >= 0 && ih < inH) {
                                for (int kw = 0; kw < kW; kw++) {
                                    int iw = ow * stride + kw * dilation - padding;
                                    if (iw >= 0 && iw < inW) {
                                        double val = input.getDouble(b, c, ih, iw);
                                        if (val > maxVal) {
                                            maxVal = val;
                                            maxIdx = ih * inW + iw;
                                        }
                        ...
                        output.setDouble(maxVal, b, c, oh, ow);
    ...
    public static DArray<Double> maxUnpool2d(DArray<Double> input, DArray<Integer> indices, int kH, int kW, int stride, int padding, int outH, int outW) {
        boolean batched = input.rank() == 4;
        if (input.rank() != 3 && input.rank() != 4) {
            throw new IllegalArgumentException("Input must be a 3D or 4D array.");
        }
        ...
        DArray<Double> output = input.dm().zeros(DType.DOUBLE, Shape.of(n, channels, outputH, outputW));
                    ...
                    for (int iw = 0; iw < inW; iw++) {
                        ...
                        if (idx >= 0) {
                            ...
                            if (oh < outputH && ow < outputW) {
                                output.setDouble(input.getDouble(b, c, ih, iw), b, c, oh, ow);
                            }
                        }
                    }
                }
    ...
    public static Pair<DArray<Double>, DArray<Integer>> maxPool3d(DArray<Double> input, int kD, int kH, int kW, int stride, int padding, int dilation, boolean ceilMode) {
        boolean batched = input.rank() == 5;
        ...
        DArray<Double> output = input.dm().zeros(DType.DOUBLE, Shape.of(n, channels, outD, outH, outW));
                ...
                for (int od = 0; od < outD; od++) {
                    for (int oh = 0; oh < outH; oh++) {
                        for (int ow = 0; ow < outW; ow++) {
                                    ...
                                    for (int kh = 0; kh < kH; kh++) {
                                        ...
                                        if (ih >= 0 && ih < inH) {
                                            for (int kw = 0; kw < kW; kw++) {
                                                int iw = ow * stride + kw * dilation - padding;
                                                if (iw >= 0 && iw < inW) {
                                                    double val = input.getDouble(b, c, id, ih, iw);
                                                    if (val > maxVal) {
                                                        maxVal = val;
                            ...
                            output.setDouble(maxVal, b, c, od, oh, ow);
    ...
    public static DArray<Double> maxUnpool3d(DArray<Double> input, DArray<Integer> indices, int kD, int kH, int kW, int stride, int padding, int outD, int outH, int outW) {
        boolean batched = input.rank() == 5;
        if (input.rank() != 4 && input.rank() != 5) {
            throw new IllegalArgumentException("Input must be a 4D or 5D array.");
        }
        ...
        DArray<Double> output = input.dm().zeros(DType.DOUBLE, Shape.of(n, channels, outputD, outputH, outputW));
                        ...
                        for (int iw = 0; iw < inW; iw++) {
                            ...
                            if (idx >= 0) {
                                ...
                                if (od < outputD && oh < outputH && ow < outputW) {
                                    output.setDouble(input.getDouble(b, c, id, ih, iw), b, c, od, oh, ow);
                                }
                            }
                        }
                    }
