...
//    }
    ...
    public DArray<Byte> conv1d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        ...
        for (int batch = 0; batch < n; batch++) {
            ...
            DArray<?> inBatch = input.selsq(0, batch); // shape: (inChannels, inLen)
            DArray<?> outBatch = output.selsq(0, batch); // shape: (outChannels, outLen)

            for (int group = 0; group < groups; group++) {

                DArray<?> inSlice = inBatch.narrow(0, group * inDepth, (group + 1) * inDepth); // shape: (inDepth, inLen)
                DArray<?> outSlice = outBatch.narrow(0, group * outDepth, (group + 1) * outDepth); // shape: (outDepth, outLen)

                DArray<?> kernelSlice = kernel.narrow(0, group * outDepth, (group + 1) * outDepth); // shape: (outDepth, inDepth, k)

                DArray<?> im2col = im2col1d(inSlice, k, padding, stride, dilation, outLen);

                im2col = im2col.reshape(Shape.of(inDepth * k, outLen));
                kernelSlice = kernelSlice.reshape(Shape.of(outDepth, inDepth * k));
                outSlice.add_(kernelSlice.mm(im2col));
            }
        }
    ...
    private DArray<Byte> im2col1d(DArray<?> inSlice, int kLen, int padding, int stride, int dilation, int outLen) {
        int inDepth = inSlice.dim(0);
        DArray<Byte> res = inSlice.dm().zeros(DType.BYTE, Shape.of(inDepth, kLen, outLen));

        for (int i = 0; i < inDepth; i++) {
            for (int k = 0; k < kLen; k++) {
                for (int o = 0; o < outLen; o++) {
                    int delta = o * stride + k * dilation - padding;
                    if (delta >= 0 && delta < inSlice.dim(1)) {
                        res.setByte(inSlice.getByte(i, delta), i, k, o);
                    }
                }
            }
        }
    ...
    @Override
    public DArray<Byte> convTranspose2d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups, int outputPadding) {
        DArray<Byte> input = this;
        while (input.rank() < 4) {
            input = input.stretch(0);
        }
    ...
    private DArray<Byte> im2col2d(DArray<?> inSlice, int kH, int kW, int padding, int stride, int dilation, int outH, int outW) {
        int inDepth = inSlice.dim(0);
        DArray<Byte> res = dm.zeros(DType.BYTE, Shape.of(inDepth, kH, kW, outH, outW));
        for (int c = 0; c < inDepth; c++) {
            for (int kh = 0; kh < kH; kh++) {
                for (int kw = 0; kw < kW; kw++) {
                    for (int oh = 0; oh < outH; oh++) {
                        for (int ow = 0; ow < outW; ow++) {
                            ...
                            if (ih >= 0 && ih < inSlice.dim(1) && iw >= 0 && iw < inSlice.dim(2)) {
                                res.setByte(inSlice.getByte(c, ih, iw), c, kh, kw, oh, ow);
                            }
                        }
                    }
                }
    ...
    public DArray<Byte> conv3d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        ...
        if (bias != null) {
            for (int oc = 0; oc < outChannels; oc++) output.narrow(1, oc, oc + 1).add_(bias.getByte(oc));
        }
        return output;
    }

    private DArray<Byte> im2col3d(DArray<?> inSlice, int kD, int kH, int kW, int padding, int stride, int dilation, int outD, int outH, int outW) {
        int inDepth = inSlice.dim(0);
        DArray<Byte> res = dm.zeros(DType.BYTE, Shape.of(inDepth, kD, kH, kW, outD, outH, outW));
        for (int c = 0; c < inDepth; c++) {
                            ...
                            for (int oh = 0; oh < outH; oh++) {
                                for (int ow = 0; ow < outW; ow++) {
                                    ...
                                    if (id >= 0 && id < inSlice.dim(1) && ih >= 0 && ih < inSlice.dim(2) && iw >= 0 && iw < inSlice.dim(3)) {
                                        res.setByte(inSlice.getByte(c, id, ih, iw), c, kd, kh, kw, od, oh, ow);
                                    }
                                }
                            }
                        }
    ...
    @Override
    public DArray<Byte> convTranspose3d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups, int outputPadding) {
        DArray<Byte> input = this;
        while (input.rank() < 5) input = input.stretch(0);
                                    ...
                                    for (int kh = 0; kh < kH; kh++) {
                                        for (int kw = 0; kw < kW; kw++) {
                                            ...
                                            if (od >= 0 && od < outD && oh >= 0 && oh < outH && ow >= 0 && ow < outW) {
                                                for (int oc = 0; oc < outDepth; oc++) {
                                                    outSlice.incByte((byte)(val * kernelSlice.getByte(c, oc, kd, kh, kw)), oc, od, oh, ow);
                                                }
                                            }
                                        }
                                    }
