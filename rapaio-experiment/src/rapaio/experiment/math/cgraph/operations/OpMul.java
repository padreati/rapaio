...
public class OpMul extends CNode {

    private final CNode left;
    private final CNode right;

    public OpMul(CompContext c, CNode left, CNode right) {
        super(c, "mul");
        this.left = left;
        this.right = right;
    }
    ...
    public List<Runnable> forward() {
        ...
        return List.of(
                () -> left.grad.add_(this.grad.get() * right.value.get()),
                () -> right.grad.add_(this.grad.get() * left.value.get())
        );
    }
}
