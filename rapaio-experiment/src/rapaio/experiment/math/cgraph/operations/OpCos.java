...
public class OpCos extends CNode {

    private final CNode child;

    public OpCos(CompContext c, CNode child) {
        super(c, "cos");
        this.child = child;
    }
    ...
    public List<Runnable> forward() {
        value.set(Math.cos(child.value().get()));
        return List.of(
                () -> child.grad().add_(this.grad.get() * (-Math.sin(child.value().get())))
        );
    }
}
