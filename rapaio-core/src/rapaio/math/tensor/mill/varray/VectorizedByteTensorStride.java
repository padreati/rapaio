...
import rapaio.math.tensor.mill.barray.BaseByteTensorStride;
...
public final class VectorizedByteTensorStride extends BaseByteTensorStride implements ByteTensor {

    private static final VectorSpecies<Byte> SPEC = ByteVector.SPECIES_PREFERRED;
    private static final int SPEC_LEN = SPEC.length();

    private final int[] loopIndexes;

    public VectorizedByteTensorStride(VectorizedArrayTensorMill mill, Shape shape, int offset, int[] strides, byte[] array) {
        this(mill, StrideLayout.of(shape, offset, strides), array);
    }

    public VectorizedByteTensorStride(VectorizedArrayTensorMill mill, Shape shape, int offset, Order order, byte[] array) {
        this(mill, StrideLayout.ofDense(shape, offset, order), array);
    }

    public VectorizedByteTensorStride(VectorizedArrayTensorMill mill, StrideLayout layout, byte[] array) {
    ...
    @Override
    public VectorizedByteTensorStride apply(Order askOrder, IntIntBiFunction<Byte> apply) {
    ...
    public ByteTensor copy(Order askOrder) {
        ...
        VectorizedByteTensorStride dst = (VectorizedByteTensorStride)
                mill.ofByte().stride(StrideLayout.ofDense(shape(), 0, askOrder), copy);
