 ...
 */
class HTMLPrinter(fileName: String, title: String, backLink: String = "") extends Printer {
  private var _textWidth: Int = 80
  private var _graphicWidth: Int = 500
  private var _graphicHeight: Int = 250
  private val _writer = new PrintWriter(new BufferedWriter(new FileWriter(new File(fileName))))

  def textWidth: Int = _textWidth

  def textWidth_=(textWidth: Int) = _textWidth = textWidth

  def graphicWidth: Int = _graphicWidth

  def graphicWidth_=(graphicWidth: Int) = _graphicWidth = graphicWidth

  def graphicHeight: Int = _graphicHeight

  def graphicHeight_=(graphicHeight: Int) = _graphicHeight = graphicHeight

  override def preparePrinter() {
    val header = Template.Header.replace(Template.KeyTitle, title).replace(Template.BackLink, backLink)
    _writer.append(header)
  }

  override def closePrinter() {
    val footer = Template.Footer.replaceAll(Template.BackLink, backLink)
    _writer.append(footer)
    _writer.flush()
    _writer.close()
  }
  ...
  def draw(figure: Figure, width: Int, height: Int) {
    ...
    val rect: Rectangle = new Rectangle(newImage.getWidth, newImage.getHeight)
    ...
    try {
      ...
      case e: IOException => throw new RuntimeException("Could not produce image", e)
    }
    _writer.append("<p><center><img src=\"data:image/png;base64," + imageString + "\" alt=\"graphics\"/></center></p>\n")
  }

  def error(message: String, throwable: Throwable) = throw new UnsupportedOperationException("Not supported yet.")

  def heading(h: Int, lines: String) = _writer.append("<h" + h + ">" + lines + "</h" + h + ">\n")
...
object Template {
  ...
  val BackLink: String = "#BACKLINK#"
  val Header: String = "<!DOCTYPE html>\n" +
    "<html><lines>\n" +
    ...
    "<script getType=\"text/javascript\">\n" +
    ...
    "</script>\n" +
    "<!-- MathJax scripts -->\n" +
    "<script getType=\"text/javascript\" src=\"https://c328740.ssl.cf1.rackcdn.com/mathjax/2.0-latest/MathJax.js?config=TeX-AMS-MML_HTMLorMML\">\n" +
    "</script>\n" +
    "</lines>\n" +
    "<body>\n" +
    "<p>#BACKLINK#</p>"
  val Footer: String = "<br/><p>#BACKLINK#</p></body></html>"
}