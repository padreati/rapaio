 ...
 */
public class HuberLossFunction implements LossFunction {
    ...
    public HuberLossFunction withAlpha(double alpha) {
        if (alpha < 0 || alpha > 1) {
    ...
    public double errorScore(Var y, Var y_hat) {
        return error(y, y_hat).darray_().nanSum();
    }
    ...
    @Override
    public boolean equalOnParams(LossFunction object) {
        if (!(object instanceof HuberLossFunction)) {
            return false;
        }
        return Double.valueOf(alpha).equals(((HuberLossFunction) object).alpha);
    }
}