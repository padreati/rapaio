...
public final class BaseByteStrideDArray extends AbstractStrideDArray<Byte> {

    public BaseByteStrideDArray(DArrayManager dm, StrideLayout layout, Storage storage) {
    ...
    @Override
    public BaseByteStrideDArray apply_(Order askOrder, IntIntBiFunction<Byte> apply) {
    ...
    public DArray<Byte> binary_(DArrayBinaryOp op, DArray<?> other) {
        ...
        DArray<Byte> o = other.cast(dt);
        Storage os = o.storage();
        Order order = Layout.storageFastTandemOrder(layout, o.layout());
        var td = TandemStrideLoopDescriptor.of(layout, (StrideLayout) o.layout(), order, Simd.vsByte);
        ...
        for (int k = 0; k < td.offsets1.length; k++) {
            ...
            if (simd) {
                if (td.step1 == 1 && td.step2 == 1) {
                    ...
                    for (; i < td.simdBound; i += td.simdLen) {
                        ByteVector a = storage.getByteVector(p1);
                        ByteVector b = os.getByteVector(p2);
                        storage.setByteVector(op.applyByte(a, b), p1);
                        p1 += td.simdLen;
                        p2 += td.simdLen;
                    }
                } else if (td.step2 == 0) {
                    // second operand is broadcast along the inner loop: load it once per inner loop
                    ByteVector b = ByteVector.broadcast(Simd.vsByte, os.getByte(p2));
                    if (td.step1 == 1) {
                        for (; i < td.simdBound; i += td.simdLen) {
                            ByteVector a = storage.getByteVector(p1);
                            storage.setByteVector(op.applyByte(a, b), p1);
                        ...
                        for (; i < td.simdBound; i += td.simdLen) {
                            ByteVector a = storage.getByteVector(p1, td.simdIdx1(), 0);
                            storage.setByteVector(op.applyByte(a, b), p1, td.simdIdx1(), 0);
                    ...
                    for (; i < td.simdBound; i += td.simdLen) {
                        ByteVector a = td.step1 == 1 ? storage.getByteVector(p1) : storage.getByteVector(p1, td.simdIdx1(), 0);
                        ByteVector b = td.step2 == 1 ? os.getByteVector(p2) : os.getByteVector(p2, td.simdIdx2(), 0);
                        ByteVector r = op.applyByte(a, b);
                        if (td.step1 == 1) {
                            storage.setByteVector(r, p1);
                        } else {
                            storage.setByteVector(r, p1, td.simdIdx1(), 0);
                        }
                        p1 += td.simdLen * td.step1;
            ...
            for (; i < td.bound; i++) {
                storage.setByte(p1, op.applyByte(storage.getByte(p1), os.getByte(p2)));
                p1 += td.step1;
                p2 += td.step2;
            }
        }
    ...
    public <M extends Number> DArray<Byte> binary_(DArrayBinaryOp op, M value) {
        ...
        ByteVector m = ByteVector.broadcast(Simd.vsByte, v);
    ...
    public DArray<Byte> fma_(Byte a, DArray<?> t) {
        ...
        byte aVal = a;
        // same data type is required for vector loads; cast() returns the same instance when types already agree
        DArray<Byte> o = t.cast(dt);
        Storage os = o.storage();
        Order order = Layout.storageFastTandemOrder(layout, o.layout());
        var td = TandemStrideLoopDescriptor.of(layout, (StrideLayout) o.layout(), order, Simd.vsByte);
        boolean simd = storage.supportSimd() && os.supportSimd();
        ByteVector av = ByteVector.broadcast(Simd.vsByte, aVal);
        ...
        for (int k = 0; k < td.offsets1.length; k++) {
            ...
            if (simd) {
                if (td.step1 == 1 && td.step2 == 1) {
                    for (; i < td.simdBound; i += td.simdLen) {
                        ByteVector x = storage.getByteVector(p1);
                        ByteVector tv = os.getByteVector(p2);
                        storage.setByteVector(Simd.fma(tv, av, x), p1);
                        p1 += td.simdLen;
                        p2 += td.simdLen;
                    }
                } else {
                    for (; i < td.simdBound; i += td.simdLen) {
                        ByteVector x = td.step1 == 1 ? storage.getByteVector(p1) : storage.getByteVector(p1, td.simdIdx1(), 0);
                        ByteVector tv = td.step2 == 1 ? os.getByteVector(p2) : os.getByteVector(p2, td.simdIdx2(), 0);
                        ByteVector r = Simd.fma(tv, av, x);
                        if (td.step1 == 1) {
                            storage.setByteVector(r, p1);
                        } else {
                            storage.setByteVector(r, p1, td.simdIdx1(), 0);
            ...
            for (; i < td.bound; i++) {
                storage.setByte(p1, Simd.fma(os.getByte(p2), aVal, storage.getByte(p1)));
                p1 += td.step1;
                p2 += td.step2;
            }
        }
    ...
    public int argmax(Order order) {
        ...
        var i = 0;
        var loop = StrideLoopDescriptor.of(layout, order, Simd.vsByte);
        for (int p : loop.offsets) {
    ...
    private Byte innerUnchecked(DArray<?> other, int start, int end) {
        StrideLoopDescriptor loop = loop();
        BaseByteStrideDArray dts = (BaseByteStrideDArray) other;
            ...
            if (simdBound > 0) {
                ...
                for (; i < simdBound; i += loop.simdLen) {
                    ByteVector v1 = step1 == 1 ?
                            ...
                            storage.getByteVector(p1, loop.simdIdx(), 0);
                    ByteVector v2 = step2 == 1 ?
                            dts.storage.getByteVector(p2) :
                            dts.storage.getByteVector(p2, otherLoop.simdIdx(), 0);
    ...
    private byte innerUncheckedUnit(BaseByteStrideDArray dts, int start, int end) {
        StrideLoopDescriptor loop = loop();
        StrideLoopDescriptor otherLoop = dts.loop();
        int i = 0;
    ...
    public DArray<Byte> bmv(DArray<?> other, Order askOrder) {
        BaseByteStrideDArray a = this;
        DArray<?> b = other;
        if (a.isScalar()) {
            a = (BaseByteStrideDArray) a.strexp(0, 1).strexp(1, 1);
        }
        if (other.isScalar()) {
            b = b.strexp(0, 1);
        }
        if (a.rank() == 2 && b.rank() == 1 && a.dim(1) == b.dim(0)) {
            ...
            return ((BaseByteStrideDArray) a.stretch(0)).bmvInternal(b.stretch(0), askOrder);
        }
        ...
        if (a.rank() == 2 && b.rank() == 2 && a.dim(1) == b.dim(1)) {
            ...
            return ((BaseByteStrideDArray) a.strexp(0, b.dim(0))).bmvInternal(b, askOrder);
        }
    ...
    public DArray<Byte> vtm(DArray<?> other, Order askOrder) {
        ...
        mvInternal((BaseByteStrideDArray) other.cast(dt).t_(), this, result);
        return result;
    }
    ...
    public DArray<?> bvtm(DArray<?> other, Order askOrder) {
        ...
        DArray<?> b = other;
        if (a.isScalar()) {
            a = (BaseByteStrideDArray) a.stretch(0);
        }
        if (other.isScalar()) {
            b = b.stretch(0, 1);
        }
        if (a.rank() == 1 && b.rank() == 2 && a.dim(0) == b.dim(0)) {
            // simple case, create a batch of 1 for each element
            return ((BaseByteStrideDArray) a.stretch(0)).bvtmInternal(b.stretch(0), askOrder);
        }
        ...
        if (a.rank() == 1 && b.rank() == 3 && a.dim(0) == b.dim(1)) {
            // batch on matrix, add batch to vector
            return ((BaseByteStrideDArray) a.strexp(0, b.dim(0))).bvtmInternal(b, askOrder);
        }
     ...
     */
    private void mvInternal(BaseByteStrideDArray a, DArray<?> x, DArray<Byte> y) {
        int m = a.dim(0);
        int n = a.dim(1);

        DArray<Byte> xc = x.cast(dt);
        ...
        if (as1 != 1 && as0 == 1 && simd) {
            ...
            for (int k = 0; k < n; k++) {
                ...
                ByteVector xkv = ByteVector.broadcast(vs, xk);
                int ptr = aOff + k * as1;
                int i = 0;
                for (; i < simdBound; i += simdLen) {
                    ByteVector col = as.getByteVector(ptr);
                    ByteVector cur = ByteVector.fromArray(vs, acc, i);
                    Simd.fma(col, xkv, cur).intoArray(acc, i);
                    ptr += simdLen;
                }
                for (; i < m; i++) {
                    acc[i] = Simd.fma(as.getByte(ptr), xk, acc[i]);
                    ptr++;
                }
            }
            int yp = yOff;
            for (int i = 0; i < m; i++) {
                y.ptrIncByte(yp, acc[i]);
                yp += yStride;
            }
            return;
        }
        ...
        for (int r0 = 0; r0 < m; r0 += rowsPerTask) {
            ...
            tasks.add(() -> {
                ...
                for (int i = rs; i < re; i++) {
                    ...
                    byte sum;
                    if (!simd) {
                        sum = rowDotScalar(as, ptr, as1, xv, 0, n, (byte) 0);
                    } else if (as1 == 1) {
                        sum = rowDotUnit(as, ptr, xv, n);
                    } else {
                        sum = rowDotGather(as, ptr, as1, gatherIdx, xv, n);
                    }
                    y.ptrIncByte(yOff + i * yStride, sum);
                }
            });
        }
    ...
    private static byte rowDotScalar(Storage as, int ptr, int step, byte[] xv, int from, int n, byte init) {
        byte sum = init;
        for (int k = from; k < n; k++) {
            sum = Simd.fma(as.getByte(ptr), xv[k], sum);
            ptr += step;
        }
        return sum;
    }

    private static byte rowDotUnit(Storage as, int ptr, byte[] xv, int n) {
        var vs = Simd.vsByte;
        int simdLen = vs.length();
        int simdBound = vs.loopBound(n);
        int twoBound = simdBound - (simdBound % (2 * simdLen));
        // two independent accumulators hide the fma latency
        ByteVector acc0 = ByteVector.zero(vs);
        ByteVector acc1 = ByteVector.zero(vs);
        int k = 0;
        for (; k < twoBound; k += 2 * simdLen) {
            acc0 = Simd.fma(as.getByteVector(ptr), ByteVector.fromArray(vs, xv, k), acc0);
            acc1 = Simd.fma(as.getByteVector(ptr + simdLen), ByteVector.fromArray(vs, xv, k + simdLen), acc1);
            ptr += 2 * simdLen;
        }
        ...
        byte sum = acc0.add(acc1).reduceLanes(VectorOperators.ADD);
        return rowDotScalar(as, ptr, 1, xv, k, n, sum);
    }

    private static byte rowDotGather(Storage as, int ptr, int step, int[] gatherIdx, byte[] xv, int n) {
        var vs = Simd.vsByte;
        int simdLen = vs.length();
        int simdBound = vs.loopBound(n);
        ByteVector acc = ByteVector.zero(vs);
        int k = 0;
        for (; k < simdBound; k += simdLen) {
            acc = Simd.fma(as.getByteVector(ptr, gatherIdx, 0), ByteVector.fromArray(vs, xv, k), acc);
            ptr += simdLen * step;
        }
        byte sum = acc.reduceLanes(VectorOperators.ADD);
    ...
    @Override
    public DArray<Byte> mm(DArray<?> other, DArray<?> to) {
    ...
    private DArray<Byte> mmBlocked(DArray<?> other, DArray<Byte> to) {
        int m = shape().dim(0);
        int n = shape().dim(1);
        int p = other.shape().dim(1);

        DArray<Byte> b = other.cast(dt);
        ...
        int simdLen = Simd.vsByte.length();
        int vec2 = 2 * simdLen;
        ...
        for (int i0 = 0; i0 < m; i0 += mc) {
            ...
            for (int j0 = 0; j0 < p; j0 += MM_NC) {
                ...
                int tj = j0;
                tasks.add(() -> {
                    byte[] ap = new byte[im * MM_KC];
                    byte[] bp = new byte[MM_KC * jnPad];
                    byte[] cp = new byte[im * jnPad];
     ...
     */
    private static void mmMicroKernel(byte[] ap, byte[] bp, byte[] cp, int im, int kn, int jnPad, int simdLen, boolean init) {
        int i = 0;
     ...
     */
    private static void mmTile4x2(byte[] ap, byte[] bp, byte[] cp, int i, int kn, int jnPad, int simdLen, boolean init) {
        var vs = Simd.vsByte;
        int vec2 = 2 * simdLen;
        {
            int a0 = i * kn;
            ...
            for (int j = 0; j < jnPad; j += vec2) {
                ByteVector acc00, acc01, acc10, acc11, acc20, acc21, acc30, acc31;
                if (init) {
                    acc00 = ByteVector.zero(vs);
                    acc01 = acc00;
                    acc10 = acc00;
                    acc11 = acc00;
                    acc20 = acc00;
                    ...
                    acc00 = ByteVector.fromArray(vs, cp, c0 + j);
                    acc01 = ByteVector.fromArray(vs, cp, c0 + j + simdLen);
                    acc10 = ByteVector.fromArray(vs, cp, c1 + j);
                    acc11 = ByteVector.fromArray(vs, cp, c1 + j + simdLen);
                    acc20 = ByteVector.fromArray(vs, cp, c2 + j);
                    acc21 = ByteVector.fromArray(vs, cp, c2 + j + simdLen);
                    acc30 = ByteVector.fromArray(vs, cp, c3 + j);
                    acc31 = ByteVector.fromArray(vs, cp, c3 + j + simdLen);
                }
                int bIdx = j;
                for (int k = 0; k < kn; k++) {
                    ByteVector b0 = ByteVector.fromArray(vs, bp, bIdx);
                    ByteVector b1 = ByteVector.fromArray(vs, bp, bIdx + simdLen);
                    ByteVector av0 = ByteVector.broadcast(vs, ap[a0 + k]);
                    ByteVector av1 = ByteVector.broadcast(vs, ap[a1 + k]);
                    ByteVector av2 = ByteVector.broadcast(vs, ap[a2 + k]);
                    ByteVector av3 = ByteVector.broadcast(vs, ap[a3 + k]);
     ...
     */
    private static void mmTile1x2(byte[] ap, byte[] bp, byte[] cp, int i, int kn, int jnPad, int simdLen, boolean init) {
        var vs = Simd.vsByte;
        int vec2 = 2 * simdLen;
        {
            int a0 = i * kn;
            ...
            for (int j = 0; j < jnPad; j += vec2) {
                ...
                ByteVector acc1;
                if (init) {
                    acc0 = ByteVector.zero(vs);
                    acc1 = acc0;
                } else {
                    acc0 = ByteVector.fromArray(vs, cp, c0 + j);
                    acc1 = ByteVector.fromArray(vs, cp, c0 + j + simdLen);
                }
                int bIdx = j;
                for (int k = 0; k < kn; k++) {
                    ByteVector av0 = ByteVector.broadcast(vs, ap[a0 + k]);
                    acc0 = Simd.fma(av0, ByteVector.fromArray(vs, bp, bIdx), acc0);
                    acc1 = Simd.fma(av0, ByteVector.fromArray(vs, bp, bIdx + simdLen), acc1);
                    bIdx += jnPad;
                }
    ...
    public DArray<Byte> bmm(DArray<?> other, Order askOrder) {
        if (rank() == 2 && other.rank() == 2 && dim(1) == other.dim(0)) {
            return ((BaseByteStrideDArray) stretch(0)).bmmInternal(other.stretch(0), askOrder);
        }
        ...
        if (rank() == 2 && other.rank() == 3 && dim(1) == other.dim(1)) {
            return ((BaseByteStrideDArray) strexp(0, other.dim(0))).bmmInternal(other, askOrder);
        }
    ...
    private DArray<Byte> bmmInternal(DArray<?> other, Order askOrder) {
        ...
        for (int b = 0; b < dim(0); b++) {
            ((BaseByteStrideDArray) selsq(0, b)).mmBlocked(other.selsq(0, b), res.selsq(0, b));
        }
        return res;
    }

    @Override
    public DArray<Byte> conv1d(DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        return BaseByteStrideDArrayConvolutions.conv1d(this, kernel, bias, stride, padding, dilation, groups);
    }

    @Override
    public DArray<Byte> convTranspose1d(DArray<?> weights, DArray<?> bias, int stride, int padding, int dilation, int groups,
            int outputPadding) {
        return BaseByteStrideDArrayConvolutions.convTranspose1d(this, weights, bias, stride, padding, dilation, groups, outputPadding);
    }

    @Override
    public DArray<Byte> unfold1d(int kLen, int stride, int padding, int dilation) {
        return BaseByteStrideDArrayConvolutions.unfold1d(this, kLen, stride, padding, dilation);
    }

    @Override
    public DArray<Byte> conv2d(DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        return BaseByteStrideDArrayConvolutions.conv2d(this, kernel, bias, stride, padding, dilation, groups);
    }

    @Override
    public DArray<Byte> unfold2d(int kH, int kW, int stride, int padding, int dilation) {
        return BaseByteStrideDArrayConvolutions.unfold2d(this, kH, kW, stride, padding, dilation);
    }

    @Override
    public DArray<Byte> convTranspose2d(DArray<?> w, DArray<?> bias, int stride, int padding, int dilation, int groups,
            int outputPadding) {
        return BaseByteStrideDArrayConvolutions.convTranspose2d(this, w, bias, stride, padding, dilation, groups, outputPadding);
    }

    @Override
    public DArray<Byte> conv3d(DArray<?> kernel, DArray<?> bias, int stride, int padding, int dilation, int groups) {
        return BaseByteStrideDArrayConvolutions.conv3d(this, kernel, bias, stride, padding, dilation, groups);
    }

    @Override
    public DArray<Byte> unfold3d(int kD, int kH, int kW, int stride, int padding, int dilation) {
        return BaseByteStrideDArrayConvolutions.unfold3d(this, kD, kH, kW, stride, padding, dilation);
    }

    @Override
    public DArray<Byte> convTranspose3d(DArray<?> weights, DArray<?> bias, int stride, int padding, int dilation, int groups,
            int outputPadding) {
        return BaseByteStrideDArrayConvolutions.convTranspose3d(this, weights, bias, stride, padding, dilation, groups, outputPadding);
    }

    @Override
    public Pair<DArray<Byte>, DArray<Integer>> maxPool1d(int kW, int stride, int padding, int dilation, boolean ceilMode) {
        return BaseByteStrideDArrayConvolutions.maxPool1d(this, kW, stride, padding, dilation, ceilMode);
    }

    @Override
    public DArray<Byte> maxUnpool1d(DArray<Byte> input, DArray<Integer> indices, int kSize, int stride, int padding, int outputSize) {
        return BaseByteStrideDArrayConvolutions.maxUnpool1d(input, indices, kSize, stride, padding, outputSize);
    }

    @Override
    public Pair<DArray<Byte>, DArray<Integer>> maxPool2d(int kH, int kW, int stride, int padding, int dilation, boolean ceilMode) {
        return BaseByteStrideDArrayConvolutions.maxPool2d(this, kH, kW, stride, padding, dilation, ceilMode);
    }

    @Override
    public DArray<Byte> maxUnpool2d(DArray<Byte> input, DArray<Integer> indices, int kH, int kW, int stride, int padding,
            int outputSizeH,
            int outputSizeW) {
        return BaseByteStrideDArrayConvolutions.maxUnpool2d(input, indices, kH, kW, stride, padding, outputSizeH, outputSizeW);
    }

    @Override
    public Pair<DArray<Byte>, DArray<Integer>> maxPool3d(int kD, int kH, int kW, int stride, int padding, int dilation,
            boolean ceilMode) {
        return BaseByteStrideDArrayConvolutions.maxPool3d(this, kD, kH, kW, stride, padding, dilation, ceilMode);
    }

    @Override
    public DArray<Byte> maxUnpool3d(DArray<Byte> input, DArray<Integer> indices, int kD, int kH, int kW, int stride, int padding,
            int outputSizeD, int outputSizeH, int outputSizeW) {
        return BaseByteStrideDArrayConvolutions.maxUnpool3d(input, indices, kD, kH, kW, stride, padding, outputSizeD, outputSizeH,
                outputSizeW);
    }
    ...
    private void sameLayoutCopy(Storage copy, Order askOrder) {
        var loop = StrideLoopDescriptor.of(layout, askOrder, Simd.vsByte);
        var last = 0;
        for (int p : loop.offsets) {
            for (int i = 0; i < loop.bound; i++) {
    ...
    public DArray<Byte> copyTo(DArray<Byte> to) {
        ...
        if (to instanceof BaseByteStrideDArray dst) {
                ...
                while (!stack.isEmpty() || loop) {
                    ...
                    if (loop) {
                        if (level == slices.length) {
                            ...
                            tasks.add(() -> {
                                BaseByteStrideDArray s = (BaseByteStrideDArray) this.narrowAll(false, ss, es);
                                BaseByteStrideDArray d = (BaseByteStrideDArray) dst.narrowAll(false, ss, es);
    ...
    private void directCopyTo(BaseByteStrideDArray src, BaseByteStrideDArray dst, Order askOrder) {
        var loop = StrideLoopDescriptor.of(src.layout, askOrder, Simd.vsByte);
