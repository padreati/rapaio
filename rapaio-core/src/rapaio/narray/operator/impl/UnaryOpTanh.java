...
public final class UnaryOpTanh extends NArrayUnaryOp {
    ...
    protected void applyUnitFloat(StrideLoopDescriptor<Float> loop, Storage s) {
        for (int p : loop.offsets) {
            ...
            for (; i < loop.simdBound; i += loop.simdLen) {
                ...
                a = a.lanewise(VectorOperators.TANH);
                s.setFloatVector(a, p);
                p += loop.simdLen;
            }
            for (; i < loop.size; i++) {
                s.setFloat(p, (float) Math.tanh(s.getFloat(p)));
                p++;
            }
        }
    }
    ...
    protected void applyUnitDouble(StrideLoopDescriptor<Double> loop, Storage s) {
        for (int p : loop.offsets) {
            ...
            for (; i < loop.size; i++) {
                s.setDouble(p, Math.tanh(s.getDouble(p)));
                p++;
            }
        }
    }
