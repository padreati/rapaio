...
import rapaio.math.tensor.engine.basearray.DTensorStride;
import rapaio.math.tensor.engine.basearray.FTensorStride;

public class ParallelArrayEngine extends AbstractEngine {

    private final ParallelArrayOfDouble ofDouble = new ParallelArrayOfDouble(this);
    private final ParallelArrayOfFloat ofFloat = new ParallelArrayOfFloat(this);
    ...
    protected static class ParallelArrayOfDouble extends AbstractOfDouble {

        private final ParallelArrayEngine parent;

        public ParallelArrayOfDouble(ParallelArrayEngine parent) {
            this.parent = parent;
        }
        ...
        public DTensor zeros(Shape shape, Order order) {
            return new rapaio.math.tensor.engine.basearray.DTensorStride(parent, shape, 0, Order.autoFC(order), new double[shape.size()]);
        }
    ...
    protected static class ParallelArrayOfFloat extends AbstractOfFloat {

        private final ParallelArrayEngine parent;

        public ParallelArrayOfFloat(ParallelArrayEngine parent) {
            this.parent = parent;
        }
        ...
        public FTensor zeros(Shape shape, Order order) {
            return new rapaio.math.tensor.engine.basearray.FTensorStride(parent, shape, 0, Order.autoFC(order), new float[shape.size()]);
        }
