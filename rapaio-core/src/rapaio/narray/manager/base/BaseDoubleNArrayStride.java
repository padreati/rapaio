...
public final class BaseDoubleNArrayStride extends AbstractStrideNArray<Double> {
    ...
    public BaseDoubleNArrayStride(NArrayManager engine, StrideLayout layout, Storage storage) {
    ...
    public NArray<Double> reshape(Shape askShape, Order askOrder) {
        ...
        NArray<Double> copy = manager.zeros(dt, askShape, askOrder);
    ...
    @Override
    public NArray<Double> reduceOp1d(NArrayReduceOp op, int axis, Order order) {
        if (axis < 0) {
            axis += shape().rank();
        }
        int[] newDims = layout.shape().narrowDims(axis);
    ...
    @Override
    public NArray<Double> varc1d(int axis, int ddof, NArray<?> mean, Order order) {
        if (axis < 0) {
            axis += shape().rank();
        }
        int[] newDims = layout.shape().narrowDims(axis);
            ...
            for (int i = 0; i < tasks; i++) {
                ...
                while (it.hasNext() && taskList.size() < chunk) {
                    ...
                    taskList.add(() -> {
                        ...
                        double value = manager.stride(dt, strideLayout, storage).reduceOp(NArrayOp.reduceVarc(ddof, m));
                        res.ptrSet(resPtr, value);
                    });
                }
                executor.submit(() -> {
    ...
    private NArray<Double> bmvInternal(NArray<?> other, Order askOrder) {
        NArray<Double> res = manager.zeros(dt, Shape.of(dim(0), dim(1)), askOrder);
    ...
    private NArray<Double> bvtmInternal(NArray<?> other, Order askOrder) {
        NArray<Double> res = manager.zeros(dt, Shape.of(dim(0), other.dim(2)), askOrder);
        for (int b = 0; b < dim(0); b++) {
    ...
    private NArray<Double> bmmInternal(NArray<?> other, Order askOrder) {
        NArray<Double> res = manager.zeros(dt, Shape.of(dim(0), dim(1), other.dim(2)), askOrder);
        for (int b = 0; b < dim(0); b++) {
    ...
    public NArray<Double> diag(int diagonal) {
        ...
        if (isVector()) {
            ...
            NArray<Double> m = manager.zeros(dt, Shape.of(n, n));
            for (int i = 0; i < dim(0); i++) {
