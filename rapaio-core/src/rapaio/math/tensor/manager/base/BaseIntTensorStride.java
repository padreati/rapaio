...
public final class BaseIntTensorStride extends AbstractStrideTensor<Integer> {
    ...
    private Tensor<Integer> mmInternal(Tensor<?> other, Tensor<Integer> to) {
        int m = shape().dim(0);
        int n = shape().dim(1);
    ...
    @Override
    public Tensor<Integer> bmm(Tensor<?> other, Order askOrder) {
        if (rank() == 2 && other.rank() == 2 && dim(1) == other.dim(0)) {
            return ((BaseIntTensorStride) stretch(0)).bmmInternal(other.stretch(0), askOrder);
        }
        ...
        if (rank() == 2 && other.rank() == 3 && dim(1) == other.dim(1)) {
            return ((BaseIntTensorStride) strexp(0, other.dim(0))).bmmInternal(other, askOrder);
        }
    ...
    private Tensor<Integer> bmmInternal(Tensor<?> other, Order askOrder) {
        Tensor<Integer> res = manager.ofInt().zeros(Shape.of(dim(0), dim(1), other.dim(2)), askOrder);
        for (int b = 0; b < dim(0); b++) {
            ((BaseIntTensorStride) takesq(0, b)).mmInternal(other.takesq(0, b), res.takesq(0, b));
        }
        return res;
    }
    ...
    @Override
    public Tensor<Integer> softmax_(int axis) {
        if (!dtype().floatingPoint()) {
    ...
    @Override
    public Tensor<Integer> logsoftmax_(int axis) {
        if (!dtype().floatingPoint()) {
            throw new IllegalArgumentException("Operation available only for float tensors.");
        }
        // TODO: this can be improved perhaps a lot
