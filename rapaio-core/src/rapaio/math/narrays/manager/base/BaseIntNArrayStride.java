...
import rapaio.math.narrays.Layout;
...
import rapaio.math.narrays.iterators.StridePointerIterator;
import rapaio.math.narrays.layout.StrideLayout;
import rapaio.math.narrays.layout.StrideWrapper;
...
public final class BaseIntNArrayStride extends AbstractStrideNArray<Integer> {
    ...
    public BaseIntNArrayStride(NArrayManager engine, StrideLayout layout, Storage<Integer> storage) {
    ...
    public NArray<Integer> reshape(Shape askShape, Order askOrder) {
        ...
        NArray<Integer> copy = manager.ofInt().zeros(askShape, askOrder);
    ...
    private NArray<Integer> bmvInternal(NArray<?> other, Order askOrder) {
        NArray<Integer> res = manager.ofInt().zeros(Shape.of(dim(0), dim(1)), askOrder);
    ...
    private NArray<Integer> bvtmInternal(NArray<?> other, Order askOrder) {
        NArray<Integer> res = manager.ofInt().zeros(Shape.of(dim(0), other.dim(2)), askOrder);
        for (int b = 0; b < dim(0); b++) {
    ...
    public NArray<Integer> mm(NArray<?> other, Order askOrder) {
        ...
        var ret = manager.ofInt().zeros(Shape.of(shape().dim(0), other.shape().dim(1)), askOrder);
    ...
    private NArray<Integer> bmmInternal(NArray<?> other, Order askOrder) {
        NArray<Integer> res = manager.ofInt().zeros(Shape.of(dim(0), dim(1), other.dim(2)), askOrder);
        for (int b = 0; b < dim(0); b++) {
    ...
    public NArray<Integer> diag(int diagonal) {
        ...
        if (isVector()) {
            ...
            NArray<Integer> m = manager.ofInt().zeros(Shape.of(n, n));
            for (int i = 0; i < dim(0); i++) {
        ...
        throw new OperationNotAvailableException("This operation is available for nArrays with shape " + shape() + ".");
    }
    ...
    @Override
    public NArray<Integer> softmax_(int axis) {
        if (!dtype().floatingPoint()) {
            throw new IllegalArgumentException("Operation available only for float NArrays.");
        }
    ...
    @Override
    public NArray<Integer> logsoftmax_(int axis) {
        if (!dtype().floatingPoint()) {
            throw new IllegalArgumentException("Operation available only for float NArrays.");
        }
        // TODO: this can be improved perhaps a lot
    ...
    protected NArray<Integer> alongAxisOperation(Order order, int axis, Function<NArray<Integer>, Integer> op) {
        ...
        NArray<Integer> res = manager.ofInt().zeros(Shape.of(newDims), Order.autoFC(order));
