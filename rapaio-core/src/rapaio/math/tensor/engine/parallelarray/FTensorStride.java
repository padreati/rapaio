...
public final class FTensorStride extends rapaio.math.tensor.engine.basearray.FTensorStride {
    ...
    public FTensorStride(Engine manager, StrideLayout layout, float[] array) {
        super(manager, layout, array);
    }

    public FTensorStride(Engine manager, Shape shape, int offset, int[] strides, float[] array) {
        this(manager, StrideLayout.of(shape, offset, strides), array);
    }

    public FTensorStride(ParallelArrayEngine manager, Shape shape, int offset, Order order, float[] array) {
    ...
    public FTensor flatten(Order askOrder) {
        ...
        return engine.ofFloat().stride(Shape.of(layout.shape().size()), 0, new int[] {1}, out);
    }
}
