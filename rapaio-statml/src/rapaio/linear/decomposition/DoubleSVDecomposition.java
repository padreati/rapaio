    ...
    public DoubleSVDecomposition(DMatrix Arg, boolean wantu, boolean wantv) {
        ...
        while (p > 0) {
            ...
            for (k = p - 2; k >= 0; k--) {
                ...
                final double threshold = tiny + MathTools.DBL_EPSILON * (abs(s.get(k)) + abs(s.get(k + 1)));
            ...
            if (k == p - 2) {
                ...
                for (ks = p - 1; ks >= k; ks--) {
                    ...
                    if (abs(s.get(ks)) <= tiny + MathTools.DBL_EPSILON * t) {
                        s.set(ks, 0.0);
                        break;
                    }
                }
        ...
        tol = Math.max(m * s.get(0) * MathTools.DBL_EPSILON, sqrt(SAFE_MIN));
    }
