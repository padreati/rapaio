...
public class Svm {
    ...
    private static SolutionInfo solve_c_svc(SvmProblem prob, SvmParameter param, double[] alpha, double cp, double cn) {
        ...
        double sumAlpha = DArrays.stride(alpha).sum();
        ...
        if (param.svmType == SvmParameter.ONE_CLASS || param.svmType == SvmParameter.EPSILON_SVR || param.svmType == SvmParameter.NU_SVR) {
            ...
            for (int i = 0; i < nr_class; i++) {
                for (int j = i + 1; j < nr_class; j++) {
                    ...
                    sub_prob.len = ci + cj;
                    sub_prob.xs = new DArray[sub_prob.len];
    ...
    public static double svm_predict(SvmModel model, DArray<Double> x) {
    ...
    public static double svm_predict_values(SvmModel model, DArray<Double> x, double[] decisionValues) {
        if (model.param.svmType == SvmParameter.ONE_CLASS ||
                model.param.svmType == SvmParameter.EPSILON_SVR ||
                model.param.svmType == SvmParameter.NU_SVR) {
    ...
    public static double svm_predict_probability(SvmModel model, DArray<Double> x, double[] probEstimates) {
