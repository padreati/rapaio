...
//        try (ExecutorService service = Executors.newFixedThreadPool(8)) {
        ...
        for (int r = 0; r < m; r++) {
            ...
            for (int c = 0; c < p; c++) {
                int cc = c;
//                    service.submit(() -> {
                var krow = (BaseDoubleDArrayStride) rows.get(rr);
                to.ptrIncDouble(off + rr * iStride + cc * jStride, (double) (krow.innerUnchecked(cols.get(cc), 0, n)));
//                        latch.countDown();
//                    });
            }
//            }
...
//    }
    ...
    @Override
    public DArray<Double> conv1d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        DArray<Double> input = this;
        ...
        for (int batch = 0; batch < n; batch++) {
            ...
            for (int group = 0; group < groups; group++) {
                ...
                DArray<Double> unfold1d = inSlice.unfold1d(k, padding, stride, dilation);
    ...
    @Override
    public DArray<Double> convTranspose1d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups,
            int outputPadding) {
        DArray<Double> input = this;
            ...
            for (int group = 0; group < groups; group++) {
                ...
                for (int i = 0; i < inLen; i++) {
                    for (int c = 0; c < inDepth; c++) {
                        double val = inSlice.getDouble(c, i);
                        for (int k = 0; k < kLen; k++) {
                            int outPos = i * stride + k * dilation - padding;
                            if (outPos >= 0 && outPos < outLen) {
                                for (int oc = 0; oc < outDepth; oc++) {
                                    outSlice.incDouble((double) (val * kernelSlice.getDouble(c, oc, k)), oc, outPos);
                                }
                            }
                        }
                    }
    ...
    @Override
    public DArray<Double> unfold1d(int kLen, int padding, int stride, int dilation) {
        ...
        int inC = input.dim(1);
        DArray<Double> result = this.dm().zeros(DType.DOUBLE, Shape.of(n, inC, kLen, outLen));
    ...
    @Override
    public DArray<Double> conv2d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        DArray<Double> input = this;
        while (input.rank() < 4) {
            input = input.stretch(0);
        }
        ...
        for (int batch = 0; batch < n; batch++) {
            DArray<Double> inBatch = input.selsq(0, batch);   // (inChannels, inH, inW)
            DArray<Double> outBatch = output.selsq(0, batch); // (outChannels, outH, outW)

            for (int group = 0; group < groups; group++) {
                DArray<Double> inSlice = inBatch.narrow(0, group * inDepth, (group + 1) * inDepth);
                DArray<Double> outSlice = outBatch.narrow(0, group * outDepth, (group + 1) * outDepth);
                DArray<?> kernelSlice = kernel.narrow(0, group * outDepth, (group + 1) * outDepth);

                // im2col: (inDepth * kH * kW, outH * outW)
                DArray<Double> col = inSlice.unfold2d(kH, kW, padding, stride, dilation);
    ...
    @Override
    public DArray<Double> unfold2d(int kH, int kW, int padding, int stride, int dilation) {
        ...
        DArray<Double> result = dm.zeros(DType.DOUBLE, Shape.of(n, inC, kH, kW, outLenH, outLenW));
        for (int b = 0; b < n; b++) {
            for (int c = 0; c < inC; c++) {
        ...
        result = result.reshape(Shape.of(n, inC * kH * kW, outLenH * outLenW));
        if (!batched) {
            result = result.squeeze(0);
        }
        return result;
    ...
    @Override
    public DArray<Double> convTranspose2d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups,
            int outputPadding) {
        DArray<Double> input = this;
        while (input.rank() < 4) {
            input = input.stretch(0);
    ...
    @Override
    public DArray<Double> conv3d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        DArray<Double> input = this;
        while (input.rank() < 5) {
            input = input.stretch(0);
        }
        ...
        DArray<Double> output = dm.zeros(DType.DOUBLE, Shape.of(n, outChannels, outD, outH, outW));
        for (int batch = 0; batch < n; batch++) {
            DArray<?> inBatch = input.selsq(0, batch);
            DArray<?> outBatch = output.selsq(0, batch);
            for (int group = 0; group < groups; group++) {
    ...
    @Override
    public DArray<Double> unfold3d(int kD, int kH, int kW, int padding, int stride, int dilation) {
        ...
        DArray<Double> result = dm.zeros(DType.DOUBLE, Shape.of(n, inCh, kD, kH, kW, outD, outH, outW));
        for (int b = 0; b < n; b++) {
    ...
    @Override
    public DArray<Double> convTranspose3d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups,
            int outputPadding) {
        DArray<Double> input = this;
        while (input.rank() < 5) {
            input = input.stretch(0);
            ...
            for (int group = 0; group < groups; group++) {
                ...
                for (int id = 0; id < inD; id++) {
                    for (int ih = 0; ih < inH; ih++) {
                        for (int iw = 0; iw < inW; iw++) {
                            for (int c = 0; c < inDepth; c++) {
                                double val = inSlice.getDouble(c, id, ih, iw);
                                for (int kd = 0; kd < kD; kd++) {
                                    for (int kh = 0; kh < kH; kh++) {
                                        for (int kw = 0; kw < kW; kw++) {
                                            ...
                                            if (od >= 0 && od < outD && oh >= 0 && oh < outH && ow >= 0 && ow < outW) {
                                                for (int oc = 0; oc < outDepth; oc++) {
                                                    outSlice.incDouble((double) (val * kernelSlice.getDouble(c, oc, kd, kh, kw)), oc, od,
                                                            oh, ow);
                                                }
                                            }
                                        }
