...
//    }
    ...
    public DArray<Byte> unfold1d(int kLen, int padding, int stride, int dilation) {
        ...
        int inC = input.dim(1);
        DArray<Byte> result = this.dm().zeros(DType.BYTE, Shape.of(n, inC, kLen, outLen));
    ...
    public DArray<Byte> conv2d(DArray<?> kernel, DArray<?> bias, int padding, int stride, int dilation, int groups) {
        ...
        for (int batch = 0; batch < n; batch++) {
            DArray<Byte> inBatch = input.selsq(0, batch);   // (inChannels, inH, inW)
            DArray<Byte> outBatch = output.selsq(0, batch); // (outChannels, outH, outW)

            for (int group = 0; group < groups; group++) {
                DArray<Byte> inSlice = inBatch.narrow(0, group * inDepth, (group + 1) * inDepth);
                DArray<Byte> outSlice = outBatch.narrow(0, group * outDepth, (group + 1) * outDepth);
                DArray<?> kernelSlice = kernel.narrow(0, group * outDepth, (group + 1) * outDepth);

                // im2col: (inDepth * kH * kW, outH * outW)
                DArray<Byte> col = inSlice.unfold2d(kH, kW, padding, stride, dilation);
    ...
    @Override
    public DArray<Byte> convTranspose2d(DArray<?> weights, DArray<?> bias, int padding, int stride, int dilation, int groups,
            int outputPadding) {
        DArray<Byte> input = this;
        while (input.rank() < 4) {
            input = input.stretch(0);
