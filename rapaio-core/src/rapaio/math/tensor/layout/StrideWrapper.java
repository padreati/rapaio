...
public final class StrideWrapper<N extends Number> extends AbstractList<N> {
    ...
    public static <N extends Number> StrideWrapper<N> of(int offset, int stride, int len, Tensor<N> tensor) {
        Function<Integer, N> getter = tensor::ptrGet;
        BiFunction<Integer, N, N> setter = (i, value) -> {
            tensor.ptrSet(i, value);
            return value;
        };
        return new StrideWrapper<>(offset, stride, len, getter, setter,
                tensor.dtype().naturalComparator(), tensor.dtype().reverseComparator());
    }
