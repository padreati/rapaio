...
public final class VectorIntTensorStride extends BaseIntTensorStride implements Tensor<Integer> {
    ...
    public VectorIntTensorStride(TensorManager engine, StrideLayout layout, Storage<Integer> storage) {
    ...
    private void binaryScalarOpUnit(TensorBinaryOp op, int value) {
        for (int off : loop.offsets) {
            int bound = VS.loopBound(loop.size);
            int i = 0;
            IntVector b = IntVector.broadcast(VS, value);
            for (; i < bound; i += VS_LEN) {
                int p = off + i * loop.step;
                IntVector a = storage.loadInt(VS, p);
                a = op.applyInt(a, b);
