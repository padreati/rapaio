 ...
 */
public class RForest extends RegressionModel<RForest, RegressionResult, RegressionHookInfo> {
            ...
            while (it.hasNext()) {
                ...
                if (future.isDone()) {
                    try {
                        ...
                        regressions.add(m);
                        runningHook.get().accept(new RegressionHookInfo(this, run++));
                        it.remove();
    ...
    protected RegressionResult corePredict(Frame df, boolean withResiduals, final double[] quantiles) {
        ...
        var pred = fit.firstPrediction().asDVector();
        pred.fill(0);
        for (VarDouble result : results) {
            pred.add(result.asDVector());
        }
