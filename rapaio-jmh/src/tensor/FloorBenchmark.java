...
public class FloorBenchmark {
    ...
    public void fillJSAT(BenchmarkState bs, Blackhole bh) {
        for (int i = 0; i < bs.jsatA.rows(); i++) {
            for (int j = 0; j < bs.jsatA.cols(); j++) {
                bs.jsatA.set(i, j, Math.floor(bs.jsatA.get(i, j)));
            }
        }
        bh.consume(bs.jsatA);
    }
    ...
    public static void main(String[] args) throws RunnerException, IOException {
        Options opt = new OptionsBuilder()
                ...
                .result(Utils.resultPath(FloorBenchmark.class))
                .build();
        new Runner(opt).run();
        Utils.resultPromote(FloorBenchmark.class);
        printResults();
    }

    public static void printResults() {
        Frame df = Csv.instance().quotes.set(true).read(Utils.resultPath(FloorBenchmark.class));
